...
- `response_format.json_schema` is required when using `type=json_object` so llm-proxy can build/enforce grammar.
...
- If grammar/json enforcement fails repeatedly, mark job `failed` and include last parse/validation error in callback
...
- LLM Proxy enforces schema when `response_format.type=json_object` and `json_schema` is provided.
